...
public class SystemNotificationSettingsServiceImpl implements SystemNotificationSettingsService {
  ...
  public NotificationSubscriptionDto upsertSubscription(UpsertNotificationSubscriptionRequest request, Authentication authentication) {
    ...
    entity.setRecipientEmail(normalizeRecipients(request.channel(), request.recipientEmail()));
  ...
  public void triggerTestNotificationEvent(TestNotificationEventRequest request, Authentication authentication) {
    ...
    MonitoringEventMutation mutation = new MonitoringEventMutation(
        action,
        normalize(request.metricName(), "arm.disk.root.used_pct"),
        "demo-trigger",
        "ARM Linux: Root disk space critical (>95%)",
        "last(/arm.disk.root.used_pct)>95",
        null,
        null,
        null,
        ThresholdLevel.HIGH,
    ...
    if (tags.isEmpty()) {
      tags = Set.of("arm-workstation");
    }
    notificationDispatchService.notifyMonitoringEvent(
        null,
        normalize(request.deviceIp(), "192.168.64.11"),
        normalize(request.deviceName(), "pilot-linux-01"),
        tags,
        mutation
    );
  }
  ...
  private static String normalizeRecipients(String channel, String input) {
    if ("TELEGRAM".equalsIgnoreCase(normalize(channel, "SMTP"))) {
      return normalizeTelegramRecipients(input);
    }
    return normalizeRecipientEmails(input);
  }

  private static String normalizeTelegramRecipients(String input) {
    ...
    for (String chatId : normalized.split(",")) {
      if (!looksLikeTelegramChatId(chatId)) {
        throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Некорректный Telegram chat id: " + chatId);
      }
    }
    return normalized;
  }
  ...
  private static boolean looksLikeTelegramChatId(String value) {
    return value.matches("-?\\d{1,20}") || value.matches("@[a-zA-Z0-9_]{5,32}");
  }
